...
import dev.oribuin.essentials.api.config.Configurable;
import dev.oribuin.essentials.util.item.ItemEdible;
import dev.oribuin.essentials.util.item.ItemEffect;
import dev.oribuin.essentials.util.item.ItemEnchant;
import dev.oribuin.essentials.util.item.ItemTexture;
import dev.oribuin.essentials.util.item.ItemValue;
...
public class ItemConstruct implements Configurable {
    ...
    public ItemStack build(StringPlaceholders placeholders) {
        ...
        if (this.name != null) stack.setData(DataComponentTypes.CUSTOM_NAME, EssUtils.kyorify(this.name, placeholders));
        if (this.lore != null) {
            List<Component> lines = new ArrayList<>();
            for (String line : this.lore) {
                String[] newLine = placeholders.apply(line).split("\n");
                for (String s : newLine) lines.add(EssUtils.kyorify(s));
            }
