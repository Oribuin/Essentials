...
public class StatementProvider {
    ...
    private StatementProvider(StatementType type, Connection connection) {
        ...
        try {
            ...
            EssentialsPlugin.getInstance().getLogger().severe("Failed to establish auto commit: " + ex.getMessage());
        }
    }
    ...
    public static StatementProvider create(StatementType type, DatabaseConnector connector) {
        try {
            ...
            EssentialsPlugin.getInstance().getLogger().severe("There was an issue establishing a ConnectionProvider: " + ex.getMessage());
    ...
    public CompletableFuture<QueryResult> execute() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                EssentialsPlugin.getInstance().getLogger().severe("Failed to execute SQL Statement: " + ex.getMessage());
                return null;
            }
        });
    }
    ...
    private void createTable() {
        ...
        try (PreparedStatement preparedStatement = this.connection.prepareStatement(statement.toString())) {
            ...
            EssentialsPlugin.getInstance().getLogger().severe("An error occurred while creating the table: " + e.getMessage());
            EssentialsPlugin.getInstance().getLogger().severe("Full Query: " + statement);
    ...
    private QueryResult select() {
        ...
        try (PreparedStatement preparedStatement = this.connection.prepareStatement(statement.toString())) {
            ...
            EssentialsPlugin.getInstance().getLogger().severe("An error occurred while selecting from the table: " + e.getMessage());
            return null;
        }
    }
    ...
    private QueryResult insert() {
        ...
        try (PreparedStatement preparedStatement = this.connection.prepareStatement(statement.toString())) {
            ...
            EssentialsPlugin.getInstance().getLogger().severe("An error occurred while inserting into the table: " + e.getMessage());
            return null;
        }
    }
    ...
    private QueryResult update() {
        if (this.columns.isEmpty()) {
            EssentialsPlugin.getInstance().getLogger().severe("Columns cannot be empty for type " + this.type.name());
            return null;
        }
        ...
        try (PreparedStatement preparedStatement = this.connection.prepareStatement(statement.toString())) {
            ...
            EssentialsPlugin.getInstance().getLogger().severe("An error occurred while updating the table: " + e.getMessage());
            return null;
        }
    }
    ...
    private QueryResult delete() {
        ...
        try (PreparedStatement preparedStatement = this.connection.prepareStatement(statement)) {
            ...
            EssentialsPlugin.getInstance().getLogger().severe("An error occurred while delete from the table: " + e.getMessage());
            return null;
        }
    }
    ...
    private void dropTable() {
        try (PreparedStatement preparedStatement = this.connection.prepareStatement("DROP TABLE IF EXISTS " + this.table)) {
            ...
            EssentialsPlugin.getInstance().getLogger().severe("An error occurred while dropping the table: " + e.getMessage());
        }
    }
